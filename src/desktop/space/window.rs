...
pub fn window_output_geometry(window: &Window, space_id: &usize, scale: impl Into<Scale<f64>>) -> Rectangle<i32, Physical> {
    let scale = scale.into();
    let loc = window_loc(window, space_id) - window.geometry().loc;
    let loc = loc.to_f64().to_physical(scale).to_i32_round();
    window.output_geometry(loc, scale)
}
...
impl Window {
    ...
    pub(super) fn elem_location(&self, space_id: usize, scale: impl Into<Scale<f64>>) -> Point<i32, Physical> {
        let loc = window_loc(self, &space_id) - self.geometry().loc;
        loc.to_f64().to_physical(scale).to_i32_round()
    }

    pub(super) fn elem_geometry(&self, space_id: usize, scale: impl Into<Scale<f64>>) -> Rectangle<i32, Physical> {
        window_output_geometry(self, &space_id, scale)
    }
