    ...
    <R as Renderer>::TextureId: 'static,
{
    ...
    pub fn location(&self, space_id: usize, scale: impl Into<Scale<f64>>) -> Point<i32, Physical> {
        match self {
            SpaceElement::Layer(layer) => layer.elem_geometry(space_id, scale).loc,
            SpaceElement::Window(window) => window.elem_location(space_id, scale),
            SpaceElement::Popup(popup) => popup.elem_geometry(space_id, scale).loc,
            SpaceElement::Custom(custom, _) => custom.geometry(scale).loc,
        }
    }
    ...
    pub fn accumulated_damage(
        ...
        match self {
            SpaceElement::Layer(layer) => layer.elem_accumulated_damage(scale, for_values),
            SpaceElement::Window(window) => window.elem_accumulated_damage(scale, for_values),
            SpaceElement::Popup(popup) => popup.elem_accumulated_damage(scale, for_values),
    ...
    <R as Renderer>::TextureId: Texture + 'static,
{
    ...
    fn accumulated_damage(
        ...
        damage_from_surface_tree(&self.surface, (0, 0), scale, for_values.map(|x| (x.0, x.1)))
    }
