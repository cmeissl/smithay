...
use crate::{
    backend::allocator::{
        dmabuf::{DmaBufSyncFlags, Dmabuf, DmabufMapping, WeakDmabuf},
        format::has_alpha,
        Buffer,
    },
...
impl PixmanImage {
    ...
    fn accessor<'l>(&'l self) -> Result<TextureAccessor<'l>, PixmanError> {
        ...
        let guard = if let Some(mapping) = self.0.dmabuf.as_ref() {
            let dmabuf = mapping.dmabuf.upgrade().ok_or(PixmanError::WlBufferDestroyed)?;
...
impl DmabufReadGuard {
    #[profiling::function]
    pub fn new(dmabuf: Dmabuf) -> std::io::Result<Self> {
...
impl<'l> TextureAccessor<'l> {
    fn with_image<F, R>(&self, f: F) -> Result<R, shm::BufferAccessError>
    where
        F: FnOnce(&'l Image<'static, 'static>) -> R,
    {
        if let Some(buffer) = self.buffer.as_ref() {
            ...
            shm::with_buffer_contents(buffer, |_, _, _| f(self.image))
        } else {
            Ok(f(self.image))
        }
    }
...
impl<'frame> PixmanFrame<'frame> {
    ...
    fn finish_internal(&mut self) -> Result<SyncPoint, PixmanError> {
        ...
        if let Some(PixmanTarget::Image { dmabuf, .. }) = self.renderer.target.as_ref() {
            dmabuf
...
impl Renderer for PixmanRenderer {
    ...
    fn render(
        ...
        if let Some(PixmanTarget::Image { dmabuf, .. }) = self.target.as_ref() {
            dmabuf
...
impl ImportMem for PixmanRenderer {
    ...
    fn update_memory(
        ...
        if texture.0 .0.buffer.is_some() || texture.0 .0.dmabuf.is_some() {
...
impl ExportMem for PixmanRenderer {
    ...
    fn copy_framebuffer(
        ...
        if let Some(target) = self.target.as_ref() {
            ...
            if let PixmanTarget::Image { dmabuf, .. } = target {
                dmabuf.sync(DmaBufSyncFlags::END | DmaBufSyncFlags::READ)?;
            };
        }
...
impl ImportDma for PixmanRenderer {
    ...
    fn import_dmabuf(
        ...
        let image = self.import_dmabuf(dmabuf, false)?;
...
impl Bind<Dmabuf> for PixmanRenderer {
    ...
    fn bind(&mut self, target: Dmabuf) -> Result<(), <Self as Renderer>::Error> {
        ...
        let image = if let Some(image) = existing_image {
            ...
            let image = self.import_dmabuf(&target, true)?;
