...
/// A element that allows re-scale another element
#[derive(Debug)]
    ...
    pub fn from_element(
        ...
        if let Some(intersection) = element_geometry.intersection(crop_rect) {
            ...
            Some(CropRenderElement {
                element,
                src,
                crop: element_relative_intersection,
...
impl<E: Element> Element for CropRenderElement<E> {
    ...
    fn geometry(&self, _scale: Scale<f64>) -> crate::utils::Rectangle<i32, Physical> {
        self.geometry
    }
    ...
    fn damage_since(
        ...
        self.element
            ...
            .into_iter()
            .flat_map(|rect| {
                rect.intersection(self.crop).map(|mut rect| {
                    rect.loc -= self.crop.loc;
                    rect
                })
            })
            .collect::<Vec<_>>()
...
/// This allows to re-locate another element
#[derive(Debug)]
