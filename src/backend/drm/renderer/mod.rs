...
use super::{DevPath, DrmSurface, PlaneClaim};
...
impl RenderElementState {
    pub(crate) fn zero_copy(visible_portion: Size<i32, Physical>) -> Self {
        RenderElementState {
            visible_portion,
...
struct FrameState<A: AsRawFd + 'static, B: AsRef<framebuffer::Handle>> {
    planes: HashMap<plane::Handle, Option<PlaneState<A, B>>>,
}
...
impl<A: AsRawFd + 'static, B: AsRef<framebuffer::Handle>> FrameState<A, B> {
    ...
    fn plane_state(&self, handle: plane::Handle) -> Option<&PlaneState<A, B>> {
        self.planes.get(&handle).and_then(|state| state.as_ref())
    }

    fn plane_state_mut(&mut self, handle: plane::Handle) -> Option<&mut PlaneState<A, B>> {
        self.planes.get_mut(&handle).and_then(|state| state.as_mut())
    }
...
impl<A: AsRawFd + 'static, B: AsRef<framebuffer::Handle>> FrameState<A, B> {
    fn test_state(
        ...
        *current_config = Some(state);

        let res = surface.test_state(
            self.planes
                .iter()
                .filter(|(_, state)| state.as_ref().map(|state| !state.skip).unwrap_or(true))
                .map(|(handle, state)| super::surface::PlaneState {
                    handle: *handle,
                    config: state.as_ref().and_then(|v| {
                        v.config.as_ref().map(|config| super::PlaneConfig {
                            src: config.src,
                            dst: config.dst,
    ...
    <F as ExportFramebuffer<B>>::Error: std::error::Error + Send + Sync,
{
        ...
        for format in SUPPORTED_FORMATS {
            ...
            match Self::find_supported_format(
                ...
                *format,
                ...
                Ok((swapchain, planes, current_frame)) => {
    ...
    > {
        ...
        match current_frame_state.test_state(&drm, planes.primary.handle, plane_state, true) {
            Ok(true) => {
                ...
                Ok((swapchain, planes, current_frame_state))
            }
            Ok(false) => {
                slog::warn!(
                    logger,
        ...
        L: Into<Option<slog::Logger>>,
    {
        ...
        // Just in case test that the primary buffer can be used for scan-out
        ...
        for (index, element) in elements.iter().enumerate() {
            ...
            let try_assign_primary_plane = if primary_plane_elements.is_empty() && remaining_elements == 1 {
                ...
                let crtc_background_matches_clear_color =
                    (clear_color[0] == 0f32 && clear_color[1] == 0f32 && clear_color[2] == 0f32)
                        || clear_color[3] == 0f32;
                (element_geometry == output_geo && element_is_opaque) || crtc_background_matches_clear_color
            } else {
                false
            };
                ...
                )
            {
                render_element_states.states.insert(
                    element_id.clone(),
                    RenderElementState::zero_copy(element_visible_size),
                );
        ...
        for (handle, previous_plane_state) in previous_state.planes.iter() {
            if let Some(previous_config) = previous_plane_state
                .as_ref()
        ...
        if render {
            ...
            match render_res {
                Ok((render_damage, mut states)) => {
                    // We need to be careful which state we extend with which one
                    // For an underlay plane we will assign an hole punch element to the primary
                    // plane with the same id as we used for the plane.
                    // To not override the zero-copy state we have to extend the dtr states
                    // with ours, the other direction would overwrite the state
                    states.states.extend(render_element_states.states);
                    render_element_states = states;
        ...
        E: RenderElement<R>,
    {
        ...
        if element_geometry.size > self.cursor_size {
            return false;
        }
        ...
        if let Some(underlying_storage) = element.underlying_storage(renderer) {
            ...
            if self.try_assign_plane(
                ...
                slog::trace!(log, "assigned element for direct scan-out on cursor plane");
                return true;
            }
        }
        ...
        let cursor_slot = match self.cursor_swapchain.acquire() {
            Ok(Some(slot)) => slot,
            Ok(None) => {
                slog::debug!(log, "failed to acquire slot for cursor plane: no free slot");
                return false;
            }
            Err(err) => {
                slog::debug!(log, "failed to acquire slot for cursor plane: {}", err);
                return false;
            }
        };
        ...
        let dmabuf = match cursor_slot.export() {
            Ok(dmabuf) => dmabuf,
            Err(err) => {
                slog::debug!(log, "failed to export cursor slot: {}", err);
                return false;
            }
        };
        ...
        if maybe_buffer.is_none() {
                ...
                .add_framebuffer(&*self.surface, ExportBuffer::Allocator(&cursor_slot))
            {
                Ok(Some(fb)) => fb,
                Ok(None) => {
                    slog::debug!(log, "failed to export framebuffer: no framebuffer available");
                    return false;
                }
                Err(err) => {
                    slog::debug!(log, "failed to export framebuffer: {}", err);
                    return false;
                }
            };
            cursor_slot
        ...
        if let Err(err) = renderer.bind(dmabuf) {
            slog::debug!(log, "failed to bind cursor buffer: {}", err);
            return false;
        };
        ...
        let (render_damage, _) = match render_res {
            Ok(res) => res,
            Err(err) => {
                slog::debug!(log, "rendering failed for cursor plane: {}", err);
        ...
        let render_damage = if previous_state
            ...
            .map(|state| state.config.is_none())
            .unwrap_or(true)
        {
        ...
        E: RenderElement<R>,
    {
        ...
        let transform = underlying_storage_transform(underlying_storage);
        ...
        let previous_commit =
            previous_state.planes.get(&plane.handle).and_then(|state| {
                state.as_ref().and_then(|state| {
                    state.element_state.as_ref().and_then(|(id, counter)| {
                        if id == element_id {
                            Some(*counter)
                        } else {
                            None
...
fn underlying_storage_transform(storage: &UnderlyingStorage) -> Transform {
    match storage {
        UnderlyingStorage::Wayland(buffer) => buffer_transform(buffer),
    }
}
...
fn plane_zpos(dev: &impl drm::control::Device, plane: plane::Handle) -> Result<Option<i32>, DrmError> {
