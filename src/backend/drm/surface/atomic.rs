...
pub struct AtomicDrmSurface<A: AsRawFd + 'static> {
    ...
    crtc: crtc::Handle,
    plane: plane::Handle,
    additional_planes: Mutex<Vec<plane::Handle>>,
...
impl<A: AsRawFd + 'static> AtomicDrmSurface<A> {
    ...
    pub fn test_state<B: AsRef<framebuffer::Handle>>(
        &self,
        planes: &[(plane::Handle, Option<PlaneConfig<B>>)],
        allow_modeset: bool,
    ) -> Result<bool, Error> {
        if !self.active.load(Ordering::SeqCst) {
            return Err(Error::DeviceInactive);
    ...
    pub fn commit<B: AsRef<framebuffer::Handle>>(
        &self,
        planes: &[(plane::Handle, Option<PlaneConfig<B>>)],
        event: bool,
    ) -> Result<(), Error> {
        ...
        if result.is_ok() {
            let mut guard = self.additional_planes.lock().unwrap();
            let planes = planes
                .iter()
                .filter(|(handle, config)| *handle != self.plane && config.is_some())
                .map(|(handle, _)| *handle)
                .collect::<Vec<_>>();
            *guard = planes;
    ...
    pub fn page_flip<B: AsRef<framebuffer::Handle>>(
        &self,
        planes: &[(plane::Handle, Option<PlaneConfig<B>>)],
        event: bool,
    ) -> Result<(), Error> {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn build_request<B: AsRef<framebuffer::Handle>>(
        &self,
        ...
        for (handle, config) in planes.iter() {
            if let Some(config) = config.as_ref() {
                ...
                req.add_property(
                    *handle,
                    ...
                    property::Value::Framebuffer(Some(*config.buffer.as_ref())),
                );
...
impl<A: AsRawFd + 'static> AsRef<framebuffer::Handle> for TestBuffer<A> {
...
impl<A: AsRawFd + 'static> Drop for AtomicDrmSurface<A> {
    fn drop(&mut self) {
        ...
        for handle in additional_planes {
            debug!(self.logger, "Clearing plane {:?} on {:?}", handle, self.crtc);
            if let Err(err) = self.clear_plane(handle) {
                warn!(
                    self.logger,
                    "Failed to clear plane {:?} on {:?}: {}", handle, self.crtc, err
                );
            }
        }
